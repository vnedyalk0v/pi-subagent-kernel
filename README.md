...
The package exports core contracts, in-memory agent and run registries, safe built-in MVP agent definitions (`scout`, `reviewer`, `tester`, `summarizer`), a `.pi/agents/*.md` loader that requires explicit project trust, and a Pi extension entrypoint that registers the canonical MVP tool names. `subagent_spawn` now returns structured mock-backend results, `subagent_status` lists or returns in-memory run status, `subagent_result` returns stored envelopes or running status, and `subagent_cancel` remains a structured placeholder until their M4 issues. `tests/extension/extension.test.ts` is the mock extension-load harness for local validation without a live Pi session.
